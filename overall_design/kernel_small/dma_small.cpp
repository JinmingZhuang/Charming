...
void address_A_ddr(axis_stream_32& addrA_out,const int TX,const int TY,const int TZ, const int D0,const int D1) {
#pragma HLS inline off
    for(ap_uint<32> dot_x=0;dot_x<D0;dot_x++){
        for(ap_uint<32> dot_y=0;dot_y<D1;dot_y++){
...
void loadA_ddr(ap_uint<AXI_WIDTH_512>* ina, axis_stream_32& addrA_in,axis_stream_512& dataA_out,const int TX,const int TY,const int TZ, const int D0,const int D1) {
#pragma HLS inline off
    ap_uint<512> temp_data;
    ap_uint<32> bound=boundA*TX*TY*TZ*D0*D1;
...
//////////////////////////////////load B//////////////////////////////////////
void address_B_ddr(axis_stream_32& addrB_out,const int TX,const int TY,const int TZ, const int D0,const int D1) {
#pragma HLS inline off    
    for(ap_uint<32> dot_x=0;dot_x<D0;dot_x++){
        for(ap_uint<32> dot_y=0;dot_y<D1;dot_y++){
            for(ap_uint<32> tx=0;tx<TX;tx++){
...
void loadB_ddr(ap_uint<AXI_WIDTH_512>* inb, axis_stream_32& addrB_in,axis_stream_512& dataB_out,const int TX,const int TY,const int TZ, const int D0,const int D1) {
#pragma HLS inline off
    ap_uint<512> temp_data;
    ap_uint<32> bound=boundB*TX*TY*TZ*D0*D1;
    for(ap_uint<32> i=0;i<bound;i++){
    #pragma HLS PIPELINE II = 1
...
void storeC(ap_uint<AXI_WIDTH_256>* c_out,ap_uint<PLIO_WIDTH> c_buf[A*C/PACKET_NUM_O][X*Z][PACKET_NUM_O][OUT_SIZE], bool enable, const int rd){
#pragma HLS inline off
    if(enable){
        for(int z=0;z<Z;z++){
            for(int j=0;j<C;j++){
                    ...
                    for(int x=0;x<X;x++){
                        for (int k = 0; k < A; k++){
                            ...
                            #pragma HLS PIPELINE II = 1
                                ...
                                temp(127,0)=c_buf[k/2][x+z*X][j+(k%2)*2][n*2+i*(H1/NUM_PER_TRA)];//(k%2)*2: first 2=(PACKET_NUM_O/C), second=C
                                temp(255,128)=c_buf[k/2][x+z*X][j+(k%2)*2][n*2+1+i*(H1/NUM_PER_TRA)];
...
void address_C_ddr(axis_stream_32& addrC_out,const int TX,const int TZ, const int D0,const int D1) {
#pragma HLS inline off
    for(ap_uint<32> dot_x=0;dot_x<D0;dot_x++){
        for(ap_uint<32> dot_y=0;dot_y<D1;dot_y++){
            for(ap_uint<32> tx=0;tx<TX;tx++){
...
void storeC_ddr(ap_uint<AXI_WIDTH_256>* outc, axis_stream_32& addrC_in,axis_stream_256& dataC_in,const int TX,const int TZ, const int D0,const int D1) { 
#pragma HLS inline off
    ap_uint<32> bound=boundC*TX*TZ*D0*D1;
    for(ap_uint<32> i=0;i<bound;i++){
    #pragma HLS PIPELINE II = 1
...
void compute(axis_stream_512& dataA_out, axis_stream_512& dataB_out, axis_stream_256& dataC_in,
    ...
    #pragma HLS bind_storage variable=buff0_A type=RAM_1P impl=LUTRAM
    ...
    #pragma HLS bind_storage variable=buff1_A type=RAM_1P impl=LUTRAM
    ...
    #pragma HLS bind_storage variable=buff0_B type=RAM_1P impl=LUTRAM
    ...
    #pragma HLS bind_storage variable=buff1_B type=RAM_1P impl=LUTRAM
