...
int main(int argc, char** argv) {
    ...
    //layer1
    xrtBufferHandle bohdl_layer1_in0 = xrtBOAlloc(dhdl, sizeIn1 * sizeof(float), 0, 0);
    auto bomapped_layer1_in0 = reinterpret_cast<float*>(xrtBOMap(bohdl_layer1_in0));
    xrtBufferHandle bohdl_layer1_in1 = xrtBOAlloc(dhdl, sizeIn2 * sizeof(float), 0, 0);
    auto bomapped_layer1_in1 = reinterpret_cast<float*>(xrtBOMap(bohdl_layer1_in1));

    //layer2
    xrtBufferHandle bohdl_layer2_in0 = xrtBOAlloc(dhdl, sizeIn1 * sizeof(float), 0, 0);
    auto bomapped_layer2_in0 = reinterpret_cast<float*>(xrtBOMap(bohdl_layer2_in0));
    xrtBufferHandle bohdl_layer2_in1 = xrtBOAlloc(dhdl, sizeIn2 * sizeof(float), 0, 0);
    auto bomapped_layer2_in1 = reinterpret_cast<float*>(xrtBOMap(bohdl_layer2_in1));

    //layer3
    xrtBufferHandle bohdl_layer3_in0 = xrtBOAlloc(dhdl, sizeIn1 * sizeof(float), 0, 0);
    auto bomapped_layer3_in0 = reinterpret_cast<float*>(xrtBOMap(bohdl_layer3_in0));
    xrtBufferHandle bohdl_layer3_in1 = xrtBOAlloc(dhdl, sizeIn2 * sizeof(float), 0, 0);
    auto bomapped_layer3_in1 = reinterpret_cast<float*>(xrtBOMap(bohdl_layer3_in1));

    //layer4
    xrtBufferHandle bohdl_layer4_in0 = xrtBOAlloc(dhdl, sizeIn1 * sizeof(float), 0, 0);
    auto bomapped_layer4_in0 = reinterpret_cast<float*>(xrtBOMap(bohdl_layer4_in0));
    xrtBufferHandle bohdl_layer4_in1 = xrtBOAlloc(dhdl, sizeIn2 * sizeof(float), 0, 0);
    auto bomapped_layer4_in1 = reinterpret_cast<float*>(xrtBOMap(bohdl_layer4_in1));

    //layer5
    xrtBufferHandle bohdl_layer5_in0 = xrtBOAlloc(dhdl, sizeIn1 * sizeof(float), 0, 0);
    auto bomapped_layer5_in0 = reinterpret_cast<float*>(xrtBOMap(bohdl_layer5_in0));
    xrtBufferHandle bohdl_layer5_in1 = xrtBOAlloc(dhdl, sizeIn2 * sizeof(float), 0, 0);
    auto bomapped_layer5_in1 = reinterpret_cast<float*>(xrtBOMap(bohdl_layer5_in1));

    //layer6
    xrtBufferHandle bohdl_layer6_in0 = xrtBOAlloc(dhdl, sizeIn1 * sizeof(float), 0, 0);
    auto bomapped_layer6_in0 = reinterpret_cast<float*>(xrtBOMap(bohdl_layer6_in0));
    xrtBufferHandle bohdl_layer6_in1 = xrtBOAlloc(dhdl, sizeIn2 * sizeof(float), 0, 0);
    auto bomapped_layer6_in1 = reinterpret_cast<float*>(xrtBOMap(bohdl_layer6_in1));

    //layer7
    xrtBufferHandle bohdl_layer7_in0 = xrtBOAlloc(dhdl, sizeIn1 * sizeof(float), 0, 0);
    auto bomapped_layer7_in0 = reinterpret_cast<float*>(xrtBOMap(bohdl_layer7_in0));
    xrtBufferHandle bohdl_layer7_in1 = xrtBOAlloc(dhdl, sizeIn2 * sizeof(float), 0, 0);
    ...
    for (int m = 0; m < M; m++) {
        for (int k = 0; k < K; k++) {
            bomapped_layer1_in0[m+k*M]=DataInput0[m][k];
        }
    }
    for (int k = 0; k < K; k++) {
        for (int n = 0; n < N; n++) {
            bomapped_layer1_in1[k+n*K]=DataInput1[k][n];
        }
    }
    ...
    for (int m = 0; m < M; m++) {
        for (int k = 0; k < K; k++) {
            bomapped_layer2_in0[m+k*M]=DataInput0[m][k];
        }
    }
    for (int k = 0; k < K; k++) {
        for (int n = 0; n < N; n++) {
            bomapped_layer2_in1[k+n*K]=DataInput1[k][n];
        }
    }
    ...
    for (int m = 0; m < M; m++) {
        for (int k = 0; k < K; k++) {
            bomapped_layer3_in0[m+k*M]=DataInput0[m][k];
        }
    }
    for (int k = 0; k < K; k++) {
        for (int n = 0; n < N; n++) {
            bomapped_layer3_in1[k+n*K]=DataInput1[k][n];
        }
    }
    ...
    for (int m = 0; m < M; m++) {
        for (int k = 0; k < K; k++) {
            bomapped_layer4_in0[m+k*M]=DataInput0[m][k];
        }
    }
    for (int k = 0; k < K; k++) {
        for (int n = 0; n < N; n++) {
            bomapped_layer4_in1[k+n*K]=DataInput1[k][n];
        }
    }
    ...
    for (int m = 0; m < M; m++) {
        for (int k = 0; k < K; k++) {
            bomapped_layer5_in0[m+k*M]=DataInput0[m][k];
        }
    }
    for (int k = 0; k < K; k++) {
        for (int n = 0; n < N; n++) {
            bomapped_layer5_in1[k+n*K]=DataInput1[k][n];
        }
    }
    ...
    for (int m = 0; m < M; m++) {
        for (int k = 0; k < K; k++) {
            bomapped_layer6_in0[m+k*M]=DataInput0[m][k];
        }
    }
    for (int k = 0; k < K; k++) {
        for (int n = 0; n < N; n++) {
            bomapped_layer6_in1[k+n*K]=DataInput1[k][n];
        }
    }
    ...
    for (int m = 0; m < M; m++) {
        for (int k = 0; k < K; k++) {
            bomapped_layer7_in0[m+k*M]=DataInput0[m][k];
        }
    }
    for (int k = 0; k < K; k++) {
        for (int n = 0; n < N; n++) {
            bomapped_layer7_in1[k+n*K]=DataInput1[k][n];
        }
    }
    ...
    //layer1
    xrtBOSync(bohdl_layer1_in0, XCL_BO_SYNC_BO_TO_DEVICE , sizeIn1* sizeof(float),0);
    xrtBOSync(bohdl_layer1_in1, XCL_BO_SYNC_BO_TO_DEVICE , sizeIn2* sizeof(float),0);

    //layer2
    xrtBOSync(bohdl_layer2_in0, XCL_BO_SYNC_BO_TO_DEVICE , sizeIn1* sizeof(float),0);
    xrtBOSync(bohdl_layer2_in1, XCL_BO_SYNC_BO_TO_DEVICE , sizeIn2* sizeof(float),0);

    //layer3
    xrtBOSync(bohdl_layer3_in0, XCL_BO_SYNC_BO_TO_DEVICE , sizeIn1* sizeof(float),0);
    xrtBOSync(bohdl_layer3_in1, XCL_BO_SYNC_BO_TO_DEVICE , sizeIn2* sizeof(float),0);

    //layer4
    xrtBOSync(bohdl_layer4_in0, XCL_BO_SYNC_BO_TO_DEVICE , sizeIn1* sizeof(float),0);
    xrtBOSync(bohdl_layer4_in1, XCL_BO_SYNC_BO_TO_DEVICE , sizeIn2* sizeof(float),0);

    //layer5
    xrtBOSync(bohdl_layer5_in0, XCL_BO_SYNC_BO_TO_DEVICE , sizeIn1* sizeof(float),0);
    xrtBOSync(bohdl_layer5_in1, XCL_BO_SYNC_BO_TO_DEVICE , sizeIn2* sizeof(float),0);

    //layer6
    xrtBOSync(bohdl_layer6_in0, XCL_BO_SYNC_BO_TO_DEVICE , sizeIn1* sizeof(float),0);
    xrtBOSync(bohdl_layer6_in1, XCL_BO_SYNC_BO_TO_DEVICE , sizeIn2* sizeof(float),0);

    //layer7
    xrtBOSync(bohdl_layer7_in0, XCL_BO_SYNC_BO_TO_DEVICE , sizeIn1* sizeof(float),0);
    xrtBOSync(bohdl_layer7_in1, XCL_BO_SYNC_BO_TO_DEVICE , sizeIn2* sizeof(float),0);
    ...
    for (int i=0;i<iter;i++){
        dma_rhdl0 = xrtKernelRun(dma_khdl0, bohdl_layer0_in0, bohdl_layer0_in1,bohdl_layer0_out,TX,TY,TZ,
                                ...
                                nullptr, nullptr, nullptr, nullptr);
        dma_rhdl1 = xrtKernelRun(dma_khdl1, bohdl_layer1_in0, bohdl_layer1_in1,bohdl_layer1_out,TX,TY,TZ,
                                nullptr, nullptr, nullptr, nullptr,
                                nullptr, nullptr, nullptr, nullptr);
        dma_rhdl2 = xrtKernelRun(dma_khdl2, bohdl_layer2_in0, bohdl_layer2_in1,bohdl_layer2_out,TX,TY,TZ,
                                nullptr, nullptr, nullptr, nullptr,
                                nullptr, nullptr, nullptr, nullptr);
        dma_rhdl3 = xrtKernelRun(dma_khdl3, bohdl_layer3_in0, bohdl_layer3_in1,bohdl_layer3_out,TX,TY,TZ,
                                nullptr, nullptr, nullptr, nullptr,
                                nullptr, nullptr, nullptr, nullptr);
        dma_rhdl4 = xrtKernelRun(dma_khdl4, bohdl_layer4_in0, bohdl_layer4_in1,bohdl_layer4_out,TX,TY,TZ,
                                nullptr, nullptr, nullptr, nullptr,
                                nullptr, nullptr, nullptr, nullptr);
        dma_rhdl5 = xrtKernelRun(dma_khdl5, bohdl_layer5_in0, bohdl_layer5_in1,bohdl_layer5_out,TX,TY,TZ,
                                nullptr, nullptr, nullptr, nullptr,
                                nullptr, nullptr, nullptr, nullptr);
        dma_rhdl6 = xrtKernelRun(dma_khdl6, bohdl_layer6_in0, bohdl_layer6_in1,bohdl_layer6_out,TX,TY,TZ,
                                nullptr, nullptr, nullptr, nullptr,
                                nullptr, nullptr, nullptr, nullptr);
        dma_rhdl7 = xrtKernelRun(dma_khdl7, bohdl_layer7_in0, bohdl_layer7_in1,bohdl_layer7_out,TX,TY,TZ,
                                nullptr, nullptr, nullptr, nullptr,
                                nullptr, nullptr, nullptr, nullptr);
